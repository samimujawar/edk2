  ...
  )
{
  ...
  // and the CurrVarArg must not be a RootNode.
  ...
  )
{
  ...
  if (PreviousLink != StartLink) {
    ...
    // We have reached the begining of the list.
    return NULL;
  }
}
...
/** Get the next sibling node among the children of the input Node.
  ...
  @return The node after the ChildNode among the children of the input Node.
           * If ChildNode is NULL, returns the first available node among
             the fixed argument list then variable list of arguments.
           * If ChildNode is the last node of the fixed argument list,
             returns the first argument of the variable list of arguments.
  ...
  if (IS_AML_OBJECT_NODE (Node)) {
    if (ChildNode == NULL) {
      ...
      if (AmlIsNodeFixedArgument (ChildNode, &Index)) {
        ...
        if (Index == AmlGetFixedArgumentCount ((AML_OBJECT_NODE*)Node)) {
  ...
  )
{
  ...
  if ((ChildNode != NULL)         &&
      IS_AML_OBJECT_NODE (Node)   &&
      (MaxIndex != AML_OP_PARSE_INDEX_GET_TERM1)) {
    if (AmlIsNodeFixedArgument (ChildNode, &Index)) {
      // The node is part of the list of fixed arguments.
      if (Index == AML_OP_PARSE_INDEX_GET_TERM1) {
        // The node is the first fixed argument, return NULL.
        return NULL;
      } else {
        // Return the previous node in the fixed argument list.
        return AmlGetFixedArgument ((AML_OBJECT_NODE*)Node, Index - 1);
      }
    }
  }
  ...
  if (CandidateNode != NULL) {
    ...
    return AmlGetFixedArgument ((AML_OBJECT_NODE*)Node, MaxIndex - 1);
  } else {
...
/** Iterate through the nodes in the same order as the AML bytestream.
  ...
  The iteration does not similar to a depth-first path.
